...
for DATASET in ${DATASETS[@]}; do
	for RESELECTION in ${RESELECTIONS[@]}; do
		for ((RUN = 1; RUN <= RUNS; RUN++)); do
			echo "python bhh.py --dataset=$DATASET --seed=$RUN --log-level=$LOG_LEVEL --population-size=5 --burn_in=0 --replay=10 --reselection=$RESELECTION --reanalysis=1 --normalise=false --credit=gbest --discounted-rewards=true"
