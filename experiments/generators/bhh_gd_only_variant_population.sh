...
for DATASET in ${DATASETS[@]}; do
	for POPULATION_SIZE in ${POPULATION_SIZES[@]}; do
		for ((RUN = 1; RUN <= RUNS; RUN++)); do
			echo "python bhh.py --variant='gd_only' --dataset=$DATASET --seed=$RUN --population-size=$POPULATION_SIZE --burn_in=0 --replay=10 --reselection=1 --reanalysis=1 --normalise=false --credit=gbest --discounted-rewards=true"
