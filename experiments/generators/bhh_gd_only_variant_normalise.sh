...
for DATASET in ${DATASETS[@]}; do
	for NORMALISE in ${NORMALISES[@]}; do
		for ((RUN = 1; RUN <= RUNS; RUN++)); do
			echo "python bhh.py --variant='gd_only' --dataset=$DATASET --seed=$RUN --log-level=$LOG_LEVEL --population-size=5 --burn_in=0 --replay=10 --reselection=1 --reanalysis=1 --normalise=$NORMALISE --credit=gbest --discounted-rewards=true"
			echo "echo 'bhh_gd_only_variant_normalise,${DATASET},${NORMALISE},${RUN}' >>tracking_bhh_gd_only_variant_normalise.txt"
		done
	done
done
