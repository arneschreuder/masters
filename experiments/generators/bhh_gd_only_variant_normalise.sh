...
for DATASET in ${DATASETS[@]}; do
	for NORMALISE in ${NORMALISES[@]}; do
		for ((RUN = 1; RUN <= RUNS; RUN++)); do
			echo "python bhh.py --variant='gd_only' --dataset=$DATASET --seed=$RUN --population-size=5 --burn_in=0 --replay=10 --reselection=1 --reanalysis=1 --normalise=$NORMALISE --credit=gbest --discounted-rewards=true"
