...
for DATASET in ${DATASETS[@]}; do
	for REANALYSIS in ${REANALYSISES[@]}; do
		for ((RUN = 1; RUN <= RUNS; RUN++)); do
			echo "python bhh.py --variant='gd_only' --dataset=$DATASET --seed=$RUN --log-level=$LOG_LEVEL --population-size=5 --burn_in=0 --replay=10 --reselection=1 --reanalysis=$REANALYSIS --normalise=false --credit=gbest --discounted-rewards=true"
