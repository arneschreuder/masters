...
for DATASET in ${DATASETS[@]}; do
	for DISCOUNTED_REWARDS in ${DISCOUNTED_REWARDSES[@]}; do
		for ((RUN = 1; RUN <= RUNS; RUN++)); do
			echo "python bhh.py --variant='gd_only'  --dataset=$DATASET --seed=$RUN --log-level=$LOG_LEVEL --population-size=5 --burn_in=0 --replay=10 --reselection=1 --reanalysis=1 --normalise=false --credit=gbest --discounted-rewards=$DISCOUNTED_REWARDSES"
