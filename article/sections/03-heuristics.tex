...
A \index{heuristic}heuristic refers to an algorithmic search technique that serves as a guide to a search process where good solutions to a optimisation problem is being sought out. Many different techniques have been used to train \acp{FFNN}~\cite{ref:kingma:2014}. At the time of writing, the majority of work that is published around the training of \acp{FFNN}, involves the use of gradient-based techniques~\cite{ref:nel:2021}. However, other techniques have also been used to successfully train \acp{FFNN}, including \acp{MH} such as \acf{PSO}~\cite{ref:rakitianskaia:2012, ref:vanwyk:2014}, \acf{DE}~\cite{ref:espinal:2011} and \acfp{GA}~\cite{ref:gupta:1999}. This research takes a particular interest in both gradient-based heuristics as well as \acp{MH}.


\subsection{Gradient-Based Heuristics}\label{sec:heuristics:gd}

Gradient-based \index{heuristic}heuristics are optimisation techniques that make use of derivates obtained from evaluating the model being optimised. There are many variants of gradient-based \index{heuristic}heuristics, however, they all fundamentally apply the same generic \acf{GD} framework called \acf{BP}.
...
% where $t_{i,p}$ refers to the $i$-th element of the target value for pattern $p$, $o_{i,p}$ refers to the $i$-th element of the output value for pattern $p$, $f$ refers to the \index{activation function}activation function, $net_{p}$ refers to the \index{net input signal}net input signal for pattern $p$, and $z_{i,p}$ refers to the input value at index $i$ for pattern $p$.
...
\subsubsection{Momentum}\label{sec:heuristics:gd:momentum}
...
\subsubsection{Nesterov Accelerated Gradients}\label{sec:heuristics:nag}
...
\subsubsection{Adaptive Gradients}\label{sec:heuristics:adagrad}
...
\subsubsection{Adaptive Learning Rate}\label{sec:heuristics:adadelta}
...
\subsubsection{Root Mean Squared Error Propagation}\label{sec:heuristics:rmsprop}
...
\subsubsection{Adaptive Moments Estimation}\label{sec:heuristics:adam}
...
\subsubsection{Particle Swarm Optimisation}\label{sec:heuristics:mh:pso}
...
\begin{algorithm}[htb]
	...
	\begin{algorithmic}
		...
		\If{$f(\boldsymbol{x}_{i}) < f(\boldsymbol{y}_{i})$ }
		\State $\boldsymbol{y}_{i} = \boldsymbol{x}_{i}$;
		\EndIf

		\State // Set the global best position;
		\If{$f(\boldsymbol{x}_{i}) < f(\boldsymbol{\hat{y}})$ }
...
\subsubsection{Differential Evolution}\label{sec:heuristics:mh:de}
...
\subsubsection{Genetic Algorithms}\label{sec:heuristics:mh:ga}
