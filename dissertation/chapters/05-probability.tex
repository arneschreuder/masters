...
\begin{figure}[htbp]
      \centering
      \includegraphics[width=0.7\textwidth]{images/additive_law_of_probability_proof.png}
...
\begin{theorem}[\textbf{Bayes' Theorem}]
      ...
      Assume that $[B_{1}, B_{2}, \dots, B_{K}]$ is a partition of $S$ such that $P(B_{k}) > 0$, for $k = 1,2, \dots, K$ then\\
      \\
      \begin{equation*}
            P(B_{k} \vert A) = \frac{P(A \vert B_{k})P(B_{k})}{\sum_{k=1}^{K} P(A \vert B_{k})P(B_{k})}
      \end{equation*}
\end{theorem}

\begin{proof}
      The proof follows directly from the definition of conditional probability as was presented in \ref{sec:probability:cond_probability}. This is shown below

      \begin{equation*}
            \begin{split}
                  P(B_{k} \vert A)
                  &= \frac{P(A \cap B_{k})}{P(A)}\\
                  &= \frac{P(A \vert B_{k})P(B_{k})}{\sum_{k=1}^{K} P(A \vert B_{k})P(B_{k})}
            \end{split}
      \end{equation*}
\end{proof}
...
\begin{figure}[htbp]
      \centering
      \includegraphics[width=0.8\textwidth]{images/dirichlet.pdf}
      \caption{The \acfp{PDF} for the Dirichlet distribution over the 2-simplex. The concentration parameter $\alpha$ is varied. The values of the \acs{PDF} are shown by the color maps with contour lines at equal values as indicated in the color bars~\cite{ref:dirichlet:2020}.}
