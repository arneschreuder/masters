...
\begin{itemize}
    ...
    \item
    \index{net input signal}\textbf{Net input signal:} The \index{net input signal}net input signal models
...
used. The normalised form of $x_{i,p}^{'}$ as determined by the \index{standard
...
Research has presented many initialisation techniques \cite{ref:erdogmus:2003}.
...
compute the \index{net input signal}net input signal as the weighted sum of all
...
An \index{activation function}activation function is used to model the action
...
\noindent Research in the past has presented many \index{activation
...
The \ac{LReLU} \index{activation function}activation function is defined as:
...
dissertation considers \index{softmax}\textit{softmax} and
...
dissertation only focuses on a \index{fully connected topology}fully connected
topology.
