...
def parse_arguments():
    ...
    # Parser
    parser = argparse.ArgumentParser(description="Analyse Case Study Data")
...
def plot_metrics(train: bool = False, accuracy=False):
    ...
    try:
        ...
        fig.suptitle(
            "BHH Case Study - {} {} - Dataset: Iris".format(DS, TYPE), fontsize=26
        )
        ...
        OUTPUT = os.path.join(
            ANALYSIS_PATH,
            "figures/{}/{}_{}.pdf".format(DS.lower(), DS.lower(), Y_DATA.lower()),
        )
...
def plot_params():
    ...
    for param_group in PARAM_GROUPS:
        ...
        for param in PARAMS:
            ...
            COLUMN_NAME = (
                param.replace("][", "_").replace("[", "_").replace("]", "_")[:-1]
            )
            try:
                fig, ax = plt.subplots(figsize=(12, 5))
                fig.suptitle(
                    "BHH Case Study - {} - Dataset: Iris".format(param), fontsize=26
                )
                ...
                OUTPUT = os.path.join(
                    ANALYSIS_PATH, "figures/{}/{}.pdf".format(param_group, param)
                )
                fig.savefig(OUTPUT, transparent=True, bbox_inches="tight")
                plt.close()
            except Exception as e:
